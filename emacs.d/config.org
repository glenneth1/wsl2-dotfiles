...
credentials.
#+BEGIN_SRC emacs-lisp 
  ;; TEST: This should appear in Messages if org-babel is working
  (message "*** ORG-BABEL TEST: Config.org is being processed! ***")
  
  (setq user-full-name "Glenn Thompson"
	user-mail-address "glenn@purplasylum.net")
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  ...
  (require 'epa-file)
  (epa-file-enable)
  (setq epa-pinentry-mode 'loopback) ;; Required if using Emacs minibuffer for pin entry
  ...
  (defun test-gpg-auth ()
    ...
    (interactive)
    (message "Testing GPG authentication...")
    (condition-case err
        (progn
          (message "Auth sources: %s" auth-sources)
          (message "GPG program: %s" epg-gpg-program)
          (message "Pinentry mode: %s" epa-pinentry-mode)
          (let ((auth-info (auth-source-search :host "chat.glennstack.dev" :user "glenn-xmpp")))
            (message "Auth search result: %s" auth-info)))
      (error (message "GPG test failed: %s" err))))
#+END_SRC
...
#+begin_src emacs-lisp
  (use-package casual-suite
    ...
    (with-eval-after-load 'saveplace
      ;; saveplace is already loaded via custom.el
      (require 'saveplace)))
#+end_src

* No Littering - Keep Emacs Directory Clean
#+begin_src emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  ;; Define WSL browser functions first (before eww configuration)
  (when (and (eq system-type 'gnu/linux)
             (getenv "WSL_DISTRO_NAME")) ; Check if running in WSL
    
    ;; Function to open URL in Windows default browser
    (defun browse-url-wsl (url &optional new-window)
      "Open URL in Windows default browser via WSL2."
      (interactive (browse-url-interactive-arg "URL: "))
      (message "Opening URL in Windows browser: %s" url)
      (let ((default-directory "/tmp")) ; Use a safe directory
        (call-process "cmd.exe" nil 0 nil "/c" "start" "" url)))
    
    ;; Function to open URL in Ubuntu Firefox (if installed)
    (defun browse-url-ubuntu-firefox (url &optional new-window)
      "Open URL in Ubuntu Firefox."
      (interactive (browse-url-interactive-arg "URL: "))
      (if (executable-find "firefox")
          (start-process "firefox" nil "firefox" url)
        (message "Firefox not found. Install with: sudo apt install firefox"))))

  (use-package eww
    :defer t
    :init
    ;; WSL-aware browser configuration using modern browse-url-handlers
    (if (and (eq system-type 'gnu/linux) (getenv "WSL_DISTRO_NAME"))
        ;; In WSL: Use Windows browser by default, eww for some sites
        (progn
          (setq browse-url-browser-function 'browse-url-wsl)  ; Default
          (setq browse-url-handlers
                '((".*google.*maps.*" . browse-url-wsl)
                  ("http.*\/\/github.com" . browse-url-wsl)
                  ("groups.google.com" . browse-url-wsl)
                  ("docs.google.com" . browse-url-wsl)
                  ("melpa.org" . browse-url-wsl)
                  ("stackoverflow\.com" . browse-url-wsl)
                  ("youtube.com" . browse-url-wsl)
                  ("amazon.com" . browse-url-wsl)
                  ("twitter.com" . browse-url-wsl)
                  ("zoom.us" . browse-url-wsl)
                  ;; Use eww for simple text-based sites
                  (".*\.txt$" . eww-browse-url)
                  (".*\.org$" . eww-browse-url))))
      ;; Not in WSL: Use original configuration
      (progn
...
#+BEGIN_SRC emacs-lisp 
  ...
  ;; Enable vertico-posframe after GUI frame is ready
  (use-package vertico-posframe
    :after vertico
    :hook (window-setup . vertico-posframe-mode)
    :custom
...
* ERC - IRC Client
#+BEGIN_SRC emacs-lisp
  ;; Enable ERC modules for highlighting and color
  (use-package erc
    :ensure t
    :custom
    (erc-modules
     '(autojoin button completion fill irccontrols list match menu move-to-prompt netsplit
       noncommands readonly ring scrolltobottom stamp track truncate
       hl-nicks colorize))  ;; Enable hl-nicks and colorize modules
    :config
    (erc-update-modules)
    (setq erc-hide-list '("JOIN" "PART" "QUIT" "MODE")))

  (defun glenn/erc-connect ()
    "Connect to ERC using credentials from .authinfo.gpg."
    (interactive)
    (let* ((credentials (car (auth-source-search
                              :host "46.62.137.191"
                              :port "8250"
                              :require '(user secret))))
           (user (plist-get credentials :user))
           (pass (let ((secret (plist-get credentials :secret)))
                   (if (functionp secret) (funcall secret) secret))))
      (if (and user pass)
          (progn
            (message "Connecting to ERC as %s..." user)
            (erc :server "46.62.137.191"
                 :port 8250
                 :nick user
                 :password pass))
        (message "Missing credentials."))))

  (global-set-key (kbd "C-q e c") #'glenn/erc-connect)
#+END_SRC

** Jabber - XMPP Client
#+BEGIN_SRC emacs-lisp
  (use-package jabber
    :straight (:host codeberg :repo "emacs-jabber/emacs-jabber" :branch "production")
    :defer t
    :init
      ...
      (when (or jabber-message-alert-same-buffer
        ...
        (if (jabber-muc-sender-p from)
            (notifications-notify
             :title (format "(PM) %s" (jabber-jid-displayname (jabber-jid-user from)))
    ...
    (jabber-account-list
     '(("glenn-xmpp@chat.glennstack.dev"
        (:network-server . "chat.glennstack.dev")
        (:port . 5222)
        (:connection-type . starttls))))
    
    ;; ERC/IRC-like appearance settings
    ...
    (jabber-muc-private-foreign-prompt-format "*%n* %t ")
    
    ;; Timestamps and formatting
    (jabber-chat-time-format "%H:%M")
    (jabber-chat-delayed-time-format "%H:%M")
    (jabber-groupchat-time-format "%H:%M")
    (jabber-muc-nick-value 'color)
    ...
    (jabber-muc-print-presence nil)
    (jabber-muc-decorate-presence-patterns nil)
    (jabber-print-rare-time nil)
    (jabber-rare-time-format "")
    
    ;; Activity tracking like ERC
    (jabber-activity-make-strings 'jabber-activity-make-strings-shorten)
    (jabber-activity-count-in-title t)
    
    :config
    ;; ERC-like faces and colors
    (set-face-foreground 'jabber-chat-prompt-local "#00ff00")
    (set-face-foreground 'jabber-chat-prompt-foreign "#0099ff")
    (set-face-foreground 'jabber-chat-text-local "#ffffff")
    (set-face-foreground 'jabber-chat-text-foreign "#cccccc")
    (set-face-foreground 'jabber-rare-time-face "#666666")
    
    ;; Roster appearance
    (set-face-attribute 'jabber-roster-user-online nil :foreground "#00ff00" :weight 'bold)
    (set-face-attribute 'jabber-roster-user-away nil :foreground "#ffff00")
    (set-face-attribute 'jabber-roster-user-xa nil :foreground "#ff6600")
    (set-face-attribute 'jabber-roster-user-dnd nil :foreground "#ff0000")
    (set-face-attribute 'jabber-roster-user-chatty nil :foreground "#00ffff")
    (set-face-attribute 'jabber-roster-user-error nil :foreground "#ff0000" :weight 'bold)
    (set-face-attribute 'jabber-roster-user-offline nil :foreground "#666666")
    
    ;; MUC (Multi-User Chat) colors - ERC-like nick coloring
    (setq jabber-muc-colorize-foreign t
          jabber-muc-colorize-local t
          ;; Enhanced nick coloring settings
          jabber-muc-nick-saturation 0.8  ; Higher saturation for more vivid colors
          jabber-muc-nick-value 0.9       ; Higher brightness for better visibility
          ;; Predefined color palette for consistent nick colors
          jabber-muc-participant-colors
          '("#ff6b6b" "#4ecdc4" "#45b7d1" "#96ceb4" "#feca57" 
            "#ff9ff3" "#54a0ff" "#5f27cd" "#00d2d3" "#ff9f43"
            "#10ac84" "#ee5a24" "#0abde3" "#feca57" "#48dbfb"
            "#ff6348" "#2ed573" "#3742fa" "#f368e0" "#ff5252"))
    
    ;; Auto-scroll like ERC
    ...
    (add-hook 'jabber-groupchat-mode-hook
              (lambda ()
                (setq-local scroll-conservatively 1000)
                (setq-local scroll-margin 3)
                (goto-address-mode 1)
    ...
    ;; Keybindings similar to ERC
    :bind (:map jabber-chat-mode-map
                ("RET" . jabber-chat-buffer-send)
                ("C-c C-c" . jabber-chat-buffer-send)
                ("C-c C-q" . bury-buffer)))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package sly
    ...
    (progn
      ...
      ;; Load log4sly setup if it exists
      (when (file-exists-p "~/quicklisp/log4sly-setup.el")
        (load "~/quicklisp/log4sly-setup.el")
        (global-log4sly-mode 1))
      ;; Load sly-stepper if it exists
      (when (file-exists-p "~/SourceCode/lisp/emacs_stuff/sly-stepper")
...
#+BEGIN_SRC emacs-lisp :tangle no
  ...
  ;; COMMENTED OUT: This was overriding auto-dark mode
  ;; (progn
...
* AI Integration - GPTel and Ollama
#+BEGIN_SRC emacs-lisp
  (use-package gptel
    :straight t
    :config
    (setf  gptel-default-mode 'org-mode
           ;; gptel-model "codellama:latest"
           gptel-model "mistral:latest" ;"qwen2.5-coder:7b"
           gptel-directives '((default . "You are a large language model living in Emacs and a helpful assistant. Respond concisely.")
                              (programming . "You are a large language model and a careful programmer and insightful system architect. Provide code and explanation as appropriate.")))
    :bind (("C-c t" . 'gptel-send)
           :map gptel-mode-map
...
#+BEGIN_SRC emacs-lisp
  ;; Configure ibuffer to show full buffer names (especially useful for XMPP room names)
  (use-package ibuffer
    :config
    (setq ibuffer-formats
          '((mark modified read-only locked " "
                  (name 120 120 :left :elide) " "
                  (size 9 -1 :right) " "
                  (mode 16 16 :left :elide) " " filename-and-process)
            (mark " " (name 120 -1) " " filename)))
    ;; Also set some additional ibuffer settings for better display
...
* Utility Functions and Keybindings
#+BEGIN_SRC emacs-lisp
  ...
  ;; Text scaling (from old config.el)
  ...
  ;; Global subword mode for better navigation
  (global-subword-mode 1)
  ...
  (when (and (eq system-type 'gnu/linux)
    ...
    (defun test-wsl-clipboard ()
      ...
      (interactive)
      (message "=== Testing WSL Clipboard Integration ===")
      (let ((test-text "WSL clipboard test - Hello from Emacs!"))
        
        ;; Test 1: Direct clip.exe test
        (message "Test 1: Testing clip.exe directly...")
        (condition-case err
            (progn
              (with-temp-buffer
                (insert test-text)
                (call-process-region (point-min) (point-max) "clip.exe" nil nil nil))
              (message "✓ clip.exe test successful"))
          (error (message "✗ clip.exe test failed: %s" err)))
        
        ;; Test 2: PowerShell Get-Clipboard test
        (message "Test 2: Testing PowerShell Get-Clipboard...")
        (condition-case err
            (let ((result (with-output-to-string
              ...
              (message "✓ PowerShell result: '%s'" (string-trim result)))
          (error (message "✗ PowerShell test failed: %s" err)))
        
        ;; Test 3: Full integration test
        (message "Test 3: Testing full integration...")
        (condition-case err
            (progn
              (funcall interprogram-cut-function test-text)
              (let ((pasted (funcall interprogram-paste-function)))
                (if (and pasted (string-match-p "WSL clipboard test" pasted))
                    (message "✓ Full integration test successful!")
                  (message "✗ Full integration test failed. Got: '%s'" pasted))))
          (error (message "✗ Full integration test failed: %s" err)))))
    
    ;; Enable clipboard integration - these are critical!
    (setq select-enable-clipboard t)
    (setq select-enable-primary t)
    (setq x-select-enable-clipboard t)  ; For older Emacs versions
    (setq x-select-enable-primary t)    ; For older Emacs versions
    
    ;; Debug: Add advice to see when kill-new is called
    (defun debug-kill-new (orig-fun &rest args)
      "Debug wrapper for kill-new to see when it's called"
      (let ((text (car args)))
        (message "DEBUG: kill-new called with: %s" (substring text 0 (min 50 (length text))))
        (apply orig-fun args)
        ;; Force clipboard copy after kill-new
        (when (and text interprogram-cut-function)
          (message "DEBUG: Forcing clipboard copy...")
          (funcall interprogram-cut-function text))))
    
    (advice-add 'kill-new :around #'debug-kill-new)
    
    ;; Also ensure clipboard integration is really enabled
    (setq select-enable-clipboard t)
    (setq select-enable-primary t)
    
    ;; Fixed clipboard copy function using the EXACT method that works!
    (setq interprogram-cut-function
          (lambda (text &optional push)
            (when text
              (message "WSL: Copying %d characters to clipboard..." (length text))
              ;; Use the exact same method as simple-copy-test (which works!)
              (let ((temp-file (make-temp-file "emacs-clip" nil ".txt")))
                (unwind-protect
                    (progn
                      (with-temp-file temp-file
                        (insert text))
                      ;; Use printf instead of echo to handle special characters better
                      (shell-command (format "printf '%%s' \"$(cat '%s')\" | clip.exe" temp-file))
                      (message "WSL: Successfully copied to clipboard"))
    ...
    (setq interprogram-paste-function
          (lambda ()
            (condition-case err
                (let ((clip-text (shell-command-to-string 
                                 "powershell.exe -noprofile -noninteractive -nologo -command 'Get-Clipboard' 2>/dev/null")))
                  (when (and clip-text (not (string= clip-text "")))
                    ;; Clean up the text - remove zsh errors and Windows line endings
                    (let ((cleaned (string-trim clip-text)))
                      ;; Extract clean text, ignoring zsh error messages
                      (if (string-match-p "can't change option\\|ERROR.*gitstatus" cleaned)
                          ;; If we have zsh errors, try to extract the actual clipboard content
                          (let ((lines (split-string cleaned "\n" t)))
                            (catch 'found-text
                              (dolist (line (reverse lines)) ; Start from the end
                                (let ((trimmed (string-trim line)))
                                  (when (and (not (string-match-p "can't change option\\|ERROR\\|Add the following\\|Restart Zsh\\|exec zsh\\|GITSTATUS" trimmed))
                                           (not (string= trimmed ""))
                                           (> (length trimmed) 0))
                                    (throw 'found-text (replace-regexp-in-string "\r\n" "\n" 
                                                                                (replace-regexp-in-string "\r$" "" trimmed))))))
                              nil))
                        ;; No zsh errors, just clean up Windows line endings
                        (replace-regexp-in-string "\r\n" "\n" 
                                                (replace-regexp-in-string "\r$" "" cleaned))))))
              (error 
               (message "WSL clipboard paste failed: %s" err)
               nil)))))

  ;; WSL2 Browser Integration - Additional functions and keybindings
  (when (and (eq system-type 'gnu/linux)
             (getenv "WSL_DISTRO_NAME")) ; Check if running in WSL
    
    ;; Function to choose browser interactively
    (defun browse-url-choose (url &optional new-window)
      "Choose which browser to open URL with."
      (interactive (browse-url-interactive-arg "URL: "))
      (let ((choice (completing-read "Open with: " 
                                     '("Windows Default" "Ubuntu Firefox" "Copy to Clipboard"))))
        (cond
         ((string= choice "Windows Default") (browse-url-wsl url))
         ((string= choice "Ubuntu Firefox") (browse-url-ubuntu-firefox url))
         ((string= choice "Copy to Clipboard") 
          (kill-new url)
          (message "URL copied to clipboard: %s" url)))))
    
    ;; Keybindings for different browser options
    (global-set-key (kbd "C-c b w") 'browse-url-wsl)           ; Windows browser
    (global-set-key (kbd "C-c b f") 'browse-url-ubuntu-firefox) ; Ubuntu Firefox
    (global-set-key (kbd "C-c b c") 'browse-url-choose)        ; Choose browser
    (global-set-key (kbd "C-c b b") 'browse-url-at-point))     ; Default browser at point

* Site-lisp Packages

** Simple Auto-dark Test
#+BEGIN_SRC emacs-lisp
  (message "SIMPLE TEST: This should appear if this block is processed")
  (add-to-list 'load-path "~/.emacs.d/site-lisp/auto-dark-emacs/")
  (require 'auto-dark)
  (message "SIMPLE TEST: Auto-dark loaded!")
  (auto-dark-mode 1)
  (message "SIMPLE TEST: Auto-dark mode enabled!")
#+END_SRC

** Auto-dark - Automatic Theme Switching
#+BEGIN_SRC emacs-lisp
  ;; CRITICAL DEBUG: This should appear in Messages if org-babel is working
  (message "*** CRITICAL DEBUG: Auto-dark section is being processed! ***")
  
  ;; Simple test: Just try to load auto-dark without conditions
  (add-to-list 'load-path "~/.emacs.d/site-lisp/auto-dark-emacs/")
  (message "DEBUG: Added auto-dark to load-path")
  
  (condition-case err
      (progn
        (require 'auto-dark)
        (message "SUCCESS: Auto-dark loaded!")
        ...
        (message "SUCCESS: Auto-dark configured")
        
        ;; Enable auto-dark mode
        (auto-dark-mode 1)
        (message "SUCCESS: Auto-dark mode enabled - command should now be available!")
        
        ;; WSL PowerShell detection fix (after auto-dark is loaded)
        (defun extract-powershell-result-wsl (output)
          "Extract numeric result from PowerShell output mixed with zsh errors"
          (let ((lines (split-string output "\n" t)))
            (catch 'found
              ;; First try: look for lines that are exactly 0 or 1
              (dolist (line lines)
                (let ((trimmed (string-trim line)))
                  (when (or (string-equal "0" trimmed) (string-equal "1" trimmed))
                    (throw 'found trimmed))))
              ;; Second try: look for any line containing only digits
              (dolist (line lines)
                (let ((trimmed (string-trim line)))
                  (when (string-match-p "^[0-9]+$" trimmed)
                    (throw 'found trimmed))))
              ;; Third try: extract last digit from the entire output
              (when (string-match "\\([01]\\)[^01]*$" output)
                (throw 'found (match-string 1 output)))
              nil)))
        
        (defun auto-dark--is-dark-mode-powershell-wsl ()
          "Fixed PowerShell detection for WSL"
          (let* ((raw-output (shell-command-to-string "powershell.exe -noprofile -noninteractive -nologo -ex bypass -command \"Get-ItemPropertyValue 'HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize' -Name AppsUseLightTheme\""))
                 (extracted-result (extract-powershell-result-wsl raw-output)))
            (and extracted-result (string-equal "0" extracted-result))))
        
        ;; Override auto-dark's PowerShell function for WSL compatibility
        (advice-add 'auto-dark--is-dark-mode-powershell :override #'auto-dark--is-dark-mode-powershell-wsl)
        (message "SUCCESS: PowerShell advice added")
        
        ;; Add hooks for theme changes
        (add-hook 'auto-dark-dark-mode-hook (lambda () (message "Auto-dark: Switched to dark theme")))
        (add-hook 'auto-dark-light-mode-hook (lambda () (message "Auto-dark: Switched to light theme")))
        (message "SUCCESS: Hooks added"))
    (error
     (message "ERROR loading auto-dark: %s" err)))

  ;; Commented out manual theme loading to allow auto-dark to work
  ;; (progn
  ;;   ;; on the terminal, the theme situation needs more attention.
  ;;   (message "Loading DeepSky theme... ")
  ;;   (load-theme 'deepsky-modus-fade t)
  ...
  ;; Debug message to confirm this section loaded
  (message "DEBUG: Debugging functions loaded successfully")
  
  ;; Ultra-simple clipboard copy test
  (defun simple-copy-test ()
    "Ultra-simple clipboard copy test"
    (interactive)
    (message "Testing simple copy...")
    (shell-command "echo 'Hello from Emacs!' | clip.exe")
    (message "✓ Executed: echo 'Hello from Emacs!' | clip.exe")
    (message "→ Try pasting in Windows now - you should see 'Hello from Emacs!'"))

  ;; Manual clipboard commands using the WORKING method
  (defun copy-to-windows-clipboard ()
    "Manually copy current region to Windows clipboard using the exact working method"
    (interactive)
    (if (region-active-p)
        (let ((text (buffer-substring-no-properties (region-beginning) (region-end))))
          (message "Copying text: '%s'" (substring text 0 (min 50 (length text))))
          
          ;; Use the EXACT same method as simple-copy-test (which works!)
          (let ((temp-file (make-temp-file "emacs-manual-clip" nil ".txt")))
            (unwind-protect
                (progn
                  (with-temp-file temp-file
                    (insert text))
                  (shell-command (format "printf '%%s' \"$(cat '%s')\" | clip.exe" temp-file))
                  (message "✓ Successfully copied %d characters to Windows clipboard!" (length text)))
              (when (file-exists-p temp-file)
                (delete-file temp-file)))))
      (message "No region selected - please select text first")))

  ;; Keyboard shortcuts for clipboard functions
  (global-set-key (kbd "C-c w") 'copy-to-windows-clipboard)     ; Copy to Windows
  (global-set-key (kbd "C-c v") 'paste-from-windows-clipboard) ; Paste from Windows
  
  ;; Alternative shortcuts that feel more natural
  (global-set-key (kbd "C-M-w") 'copy-to-windows-clipboard)    ; Ctrl+Alt+W
  (global-set-key (kbd "C-M-y") 'paste-from-windows-clipboard) ; Ctrl+Alt+Y
  
  (defun paste-from-windows-clipboard ()
    "Manually paste from Windows clipboard"
    (interactive)
    (let ((text (funcall interprogram-paste-function)))
      (if text
          (progn
            (insert text)
            (message "Pasted %d characters from Windows clipboard" (length text)))
        (message "Windows clipboard is empty"))))

  ;; Simple clipboard verification test
  (defun test-clipboard-step-by-step ()
    "Test clipboard functionality step by step with verification"
    (interactive)
    (message "=== Step-by-Step Clipboard Test ===")
    
    ;; Step 1: Write a known value to clipboard
    (message "Step 1: Writing 'EMACS-TEST-123' to clipboard...")
    (shell-command "echo 'EMACS-TEST-123' | clip.exe")
    (message "✓ Command executed")
    
    ;; Step 2: Try to read it back
    (message "Step 2: Reading clipboard back...")
    (let ((result (shell-command-to-string "powershell.exe -noprofile -noninteractive -nologo -command 'Get-Clipboard' 2>/dev/null")))
      (message "Raw clipboard result: '%s'" result)
      (let ((cleaned (string-trim result)))
        (message "Cleaned result: '%s'" cleaned)
        (if (string-match-p "EMACS-TEST-123" cleaned)
            (message "✓ SUCCESS: Clipboard round-trip works!")
          (message "✗ FAILED: Expected 'EMACS-TEST-123', got '%s'" cleaned))))
    
    (message "→ Now manually try pasting in Windows - you should see 'EMACS-TEST-123'"))

  ;; Test Windows commands directly
  (defun test-windows-commands ()
    "Test if Windows commands are accessible from WSL"
    (interactive)
    (message "=== Testing Windows Command Access ===")
    
    ;; Test 1: Can we run clip.exe?
    (message "Test 1: Testing clip.exe accessibility...")
    (condition-case err
        (progn
          (call-process "clip.exe" nil nil nil)
          (message "✓ clip.exe is accessible"))
      (error (message "✗ clip.exe failed: %s" err)))
    
    ;; Test 2: Can we run powershell.exe?
    (message "Test 2: Testing powershell.exe accessibility...")
    (condition-case err
        (let ((result (shell-command-to-string "powershell.exe -command 'Write-Output \"test\"'")))
          (message "✓ PowerShell result: %s" (string-trim result)))
      (error (message "✗ PowerShell failed: %s" err)))
    
    ;; Test 3: Can we run cmd.exe?
    (message "Test 3: Testing cmd.exe accessibility...")
    (condition-case err
        (let ((result (shell-command-to-string "cmd.exe /c echo test")))
          (message "✓ cmd.exe result: %s" (string-trim result)))
      (error (message "✗ cmd.exe failed: %s" err)))
    
    ;; Test 4: Test simple clipboard operation
    (message "Test 4: Testing simple clipboard write...")
    (condition-case err
        (progn
          (shell-command "echo 'WSL test' | clip.exe")
          (message "✓ Simple clipboard write completed")
          (message "→ Try pasting in Windows to see if this worked"))
      (error (message "✗ Simple clipboard write failed: %s" err))))

  ;; Test M-w specifically
  (defun test-mw-integration ()
    "Test what happens when we use M-w"
    (interactive)
    (message "=== Testing M-w Integration ===")
    (message "Current kill-ring length: %d" (length kill-ring))
    (message "interprogram-cut-function is: %s" (if interprogram-cut-function "SET" "NOT SET"))
    (message "select-enable-clipboard: %s" select-enable-clipboard)
    (message "Now select some text and press M-w, then check messages..."))

  ;; Debug clipboard integration
  (defun debug-clipboard-integration ()
    "Debug why M-w and C-y aren't working with Windows clipboard"
    (interactive)
    (message "=== Debugging Clipboard Integration ===")
    (message "interprogram-cut-function: %s" interprogram-cut-function)
    (message "interprogram-paste-function: %s" interprogram-paste-function)
    (message "select-enable-clipboard: %s" select-enable-clipboard)
    (message "select-enable-primary: %s" select-enable-primary)
    (message "x-select-enable-clipboard: %s" x-select-enable-clipboard)
    (message "x-select-enable-primary: %s" x-select-enable-primary)
    (message "WSL_DISTRO_NAME: %s" (getenv "WSL_DISTRO_NAME"))
    (message "System type: %s" system-type))

  ;; Quick clipboard test function
  (defun quick-clipboard-test ()
    "Quick test of clipboard functionality"
    (interactive)
    (let ((test-text "Quick clipboard test from Emacs - Hello Windows!"))
      (message "=== Testing Clipboard Copy (Emacs → Windows) ===")
      (message "Testing multiple methods...")
      
      ;; Method 1: Test direct clip.exe
      (message "Method 1: Direct clip.exe...")
      (condition-case err
          (progn
            (with-temp-buffer
              (insert test-text)
              (call-process-region (point-min) (point-max) "clip.exe" nil nil nil))
            (message "✓ Direct clip.exe successful"))
        (error (message "✗ Direct clip.exe failed: %s" err)))
      
      ;; Method 2: Test cmd.exe with echo
      (message "Method 2: cmd.exe with echo...")
      (condition-case err
          (progn
            (call-process "cmd.exe" nil nil nil "/c" (format "echo %s | clip" test-text))
            (message "✓ cmd.exe echo method successful"))
        (error (message "✗ cmd.exe echo method failed: %s" err)))
      
      ;; Method 3: Test the actual interprogram-cut-function
      (message "Method 3: Using interprogram-cut-function...")
      (condition-case err
          (progn
            (funcall interprogram-cut-function test-text)
            (message "✓ interprogram-cut-function executed"))
        (error (message "✗ interprogram-cut-function failed: %s" err)))
      
      ;; Method 4: Test kill-new (what M-w uses)
      (message "Method 4: Using kill-new...")
      (kill-new test-text)
      (message "✓ kill-new executed")
      
      ;; Method 5: Test what happens when we copy/paste
      (message "Method 5: Testing actual M-w workflow...")
      (with-temp-buffer
        (insert test-text)
        (mark-whole-buffer)
        (kill-ring-save (point-min) (point-max))
        (message "✓ Simulated M-w (kill-ring-save) executed"))
      
      (message "→ Now try pasting (Ctrl+V) in a Windows app to see which method worked.")
      (message "→ Then copy something in Windows and press C-y in Emacs to test paste.")))
  
  ;; Quick browser test function  
  (defun quick-browser-test ()
    "Quick test of browser functionality"
    (interactive)
    (let ((test-url "https://www.google.com"))
      (message "=== Testing Browser Integration ===")
      (message "→ Opening URL: %s" test-url)
      (condition-case err
          (progn
            (browse-url test-url)
            (message "✓ Browser command executed successfully"))
        (error (message "✗ Browser test failed: %s" err)))))

  ;; Simple debugging functions (available even if auto-dark fails to load)
  (defun test-powershell-simple ()
    "Simple test of PowerShell detection."
    (interactive)
    (let* ((output (shell-command-to-string "powershell.exe -noprofile -noninteractive -nologo -ex bypass -command \"Get-ItemPropertyValue 'HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize' -Name AppsUseLightTheme\""))
           (extracted (when (fboundp 'extract-powershell-result-wsl)
                        (extract-powershell-result-wsl output))))
      (message "PowerShell raw output: %s" output)
      (message "Length: %d" (length output))
      (message "Extracted result: %s" (or extracted "NONE"))
      (message "Is dark mode: %s" (if extracted (string-equal "0" extracted) "UNKNOWN"))))

  (defun check-auto-dark-status ()
    "Check if auto-dark is loaded and working."
    (interactive)
    (if (featurep 'auto-dark)
        (message "Auto-dark is loaded. Mode active: %s" (if (bound-and-true-p auto-dark-mode) "YES" "NO"))
      (message "Auto-dark is NOT loaded")))

  (defun manual-theme-test ()
    "Manually test theme switching."
    (interactive)
    (let ((choice (completing-read "Load theme: " '("doom-one" "doom-one-light" "wombat" "adwaita"))))
      (load-theme (intern choice) t)
      (message "Loaded theme: %s" choice)))

  (defun debug-auto-dark-loading ()
    "Debug auto-dark loading issues."
    (interactive)
    (let ((auto-dark-file "~/.emacs.d/site-lisp/auto-dark-emacs/auto-dark.el"))
      (message "=== Auto-dark Debug Info ===")
      (message "File exists: %s" (file-exists-p auto-dark-file))
      (message "Load path contains auto-dark: %s" 
               (member (expand-file-name "~/.emacs.d/site-lisp/auto-dark-emacs/") load-path))
      (message "Auto-dark feature loaded: %s" (featurep 'auto-dark))
      (message "Auto-dark mode bound: %s" (fboundp 'auto-dark-mode))
      (when (fboundp 'auto-dark-mode)
        (message "Auto-dark mode active: %s" (if (bound-and-true-p auto-dark-mode) "YES" "NO")))
      (message "Current themes: %s" (mapcar 'symbol-name custom-enabled-themes))))
#+END_SRC

** LASS - CSS Generation
#+BEGIN_SRC emacs-lisp
  ;; LASS - Lisp Augmented Style Sheets (from old config.el)
  (add-to-list 'load-path "/home/glenn/.emacs.d/site-lisp/LASS/")
  (require 'lass)
#+END_SRC

** Emacs Reader - Document Reading Mode
#+BEGIN_SRC emacs-lisp
  ;; Configure emacs-reader
  (use-package reader
    :load-path "~/.emacs.d/site-lisp/emacs-reader"
    :init
    ;; Remove the saveplace advice to avoid errors if they occur
    (with-eval-after-load 'saveplace
      (advice-remove 'save-place-find-file-hook #'reader--saveplace-find-file)
      (advice-remove 'save-place-to-alist #'reader--saveplace-to-alist))
    
    ;; Stability mode by default (no threads, fewer pages)
    (setq reader-num-threads 0)
    (setq reader-page-rendering-max-pages 1)
    (setq reader-page-rendering-timeout 10)
    
    ;; Toggle functions for reader performance/stability
    (defun reader-performance-mode ()
      "Enable performance mode for reader (more threads, more pages)."
      (interactive)
      (setq reader-num-threads 2)
      (setq reader-page-rendering-max-pages 4)
      (setq reader-page-rendering-timeout 15)
      (message "Reader performance mode enabled - use with caution, may cause crashes"))
    
    (defun reader-stability-mode ()
      "Enable stability mode for reader (no threads, fewer pages)."
      (interactive)
      (setq reader-num-threads 0)
      (setq reader-page-rendering-max-pages 1)
      (setq reader-page-rendering-timeout 10)
      (message "Reader stability mode enabled"))
    
    ;; Experimental mode with just 1 thread
    (defun reader-balanced-mode ()
      "Enable balanced mode for reader (1 thread, 2 pages)."
      (interactive)
      (setq reader-num-threads 1)
      (setq reader-page-rendering-max-pages 2)
      (setq reader-page-rendering-timeout 12)
      (message "Reader balanced mode enabled - experimental"))
    
    :bind (:map reader-mode-map
                ("C-c C-p" . reader-performance-mode)
                ("C-c C-s" . reader-stability-mode)
                ("C-c C-b" . reader-balanced-mode))
    :mode ("\\.pdf\\'" "\\.epub\\'" "\\.mobi\\'" "\\.fb2\\'" "\\.xps\\'" "\\.cbz\\'"))

  ;; Helper function for generating autoloads for site-lisp packages
  (defun my/generate-autoloads-for-directory (dir)
    "Generate autoloads for all elisp files in DIR."
    (interactive "DDirectory: ")
    (let* ((dir (expand-file-name dir))
           (package-name (file-name-nondirectory (directory-file-name dir)))
           (generated-autoload-file (expand-file-name (format "%s-autoloads.el" package-name) dir)))
      (when (file-exists-p generated-autoload-file)
        (delete-file generated-autoload-file))
      (message "Generating autoloads for %s..." package-name)
      (package-generate-autoloads package-name dir)
      (message "Generating autoloads for %s...done" package-name)))
#+END_SRC
